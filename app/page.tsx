...
import { Grade, Course, Semester, calculateSGPA, calculateCGPA, gradePoints, defaultPresetCredits } from '../utils/gradeCalculations'
...
export default function CGPACalculator() {
  const [semesters, setSemesters] = useState<Semester[]>([{ courses: [{ name: '', credits: 0, grade: 'O' }] }])
  const [cgpa, setCGPA] = useState<number | null>(null)
  const [presetModes, setPresetModes] = useState<boolean[]>([false])

  const addSemester = () => {
    setSemesters([...semesters, { courses: [{ name: '', credits: 0, grade: 'O' }] }])
    setPresetModes([...presetModes, false])
  }

  const addCourse = (semesterIndex: number) => {
    const newSemesters = [...semesters]
    newSemesters[semesterIndex].courses.push({ name: '', credits: 0, grade: 'O' })
  ...
  const updateSemester = (semesterIndex: number, field: 'sgpa' | 'totalCredits', value: number) => {
    const newSemesters = [...semesters]
  ...
  const toggleSGPAInput = (semesterIndex: number) => {
    ...
    if (newSemesters[semesterIndex].sgpa !== undefined) {
      ...
      newSemesters[semesterIndex].totalCredits = defaultPresetCredits[semesterIndex] || 20
  ...
  const togglePreset = (semesterIndex: number) => {
    ...
    if (newPresetModes[semesterIndex]) {
      ...
      newSemesters[semesterIndex] = { courses: [{ name: '', credits: 0, grade: 'O' }] }
    }
    setSemesters(newSemesters)
    setPresetModes(newPresetModes)
  }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {semesters.map((semester, semesterIndex) => (
        <Card key={semesterIndex} className="mb-4">
          <CardHeader>
            ...
            <CardDescription>Enter course details, SGPA, or use preset</CardDescription>
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor={`sgpa-toggle-${semesterIndex}`}>I know my SGPA</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Switch
                ...
                />
                <Label htmlFor={`preset-toggle-${semesterIndex}`}>Use Preset</Label>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {presetModes[semesterIndex] ? (
              <div className="flex space-x-2 mb-2">
                <Input
                  ...
                  placeholder="SGPA"
                  value={semester.sgpa || ''}
                  onChange={(e) => updateSemester(semesterIndex, 'sgpa', Number(e.target.value))}
                  className="w-1/2"
                />
                <Input
                  type="number"
                  ...
                  onChange={(e) => updateSemester(semesterIndex, 'totalCredits', Number(e.target.value))}
                  className="w-1/2"
                />
              </div>
              ...
              semester.courses.map((course, courseIndex) => (
                <div key={courseIndex} className="flex space-x-2 mb-2">
                  ...
                  <Select
                    value={course.grade}
                    onValueChange={(value: Grade) => updateCourse(semesterIndex, courseIndex, 'grade', value)}
                  >
          ...
          <CardFooter className="flex justify-between">
            {!presetModes[semesterIndex] && semester.sgpa === undefined && (
              <Button onClick={() => addCourse(semesterIndex)}>Add Course</Button>
            )}
      ...
      <Button onClick={addSemester} className="mb-4 mr-4">Add Semester</Button>
      <Button onClick={calculateResults} className="mb-4">Calculate CGPA</Button>
      {cgpa !== null && (
        <Card>
          <CardHeader>
